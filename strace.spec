...
# RHEL-only
Patch0: 0000-limit-qual_fault-scope-on-aarch64.patch
# v5.2-3-g7ada13f "evdev: avoid bit vector decoding on non-successful and 0 return codes"
Patch1: 0001-evdev-avoid-bit-vector-decoding-on-non-successful-an.patch
# v5.2-4-g96194ed "evdev: fix array size calculation in decode_bitset_"
Patch2: 0002-evdev-fix-array-size-calculation-in-decode_bitset_.patch
# v5.2-5-gcdd8206 "tests: test evdev bitset decoding more thoroughly"
Patch3: 0003-tests-test-evdev-bitset-decoding-more-thoroughly.patch
# v5.2-92-gc108f0b "sockaddr: properly decode sockaddr_hci addresses without hci_channel"
Patch4: 0004-sockaddr-properly-decode-sockaddr_hci-addresses-with.patch
## Reported by covscan
# v5.2-84-g91281fec "v4l2: avoid shifting left a signed number by 31 bit"
Patch5: 0005-v4l2-avoid-shifting-left-a-signed-number-by-31-bit.patch
# v5.2~21 "syscall.c: avoid infinite loop in subcalls parsing"
Patch6: 0006-syscall.c-avoid-infinite-loop-in-subcalls-parsing.patch
# v5.2~19 "kvm: avoid bogus vcpu_info assignment in vcpu_register"
Patch7: 0007-kvm-avoid-bogus-vcpu_info-assignment-in-vcpu_registe.patch

## RHEL-only: headers on some builders do not provide O_TMPFILE
Patch10: strace-provide-O_TMPFILE-fallback-definition.patch
...
%if 0%{?fedora} >= 18 || 0%{?centos} >= 6 || 0%{?rhel} >= 6 || 0%{?suse_version} >= 1200
...
# For now, it seems that the failures on s390x builders (that, for some reason,
# use el6 kernel) are covered by the following issues:
#  * https://bugzilla.redhat.com/show_bug.cgi?id=1220802#c6
